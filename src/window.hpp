...
public:
  ...
  VkSurfaceKHR &getVkSurface() { return vulkanSurface; }
  ...
  void initWindow(GLFWframebuffersizefun framebufferResizeCallback) {
    ...
    if (FULLSCREEN_ENABLED) {
      ...
      glfwWindow = glfwCreateWindow(window_width, window_height, "Vulkan",
  ...
  /// button)
  const bool shouldClose() const { return glfwWindowShouldClose(glfwWindow); }

  void createVulkanSurface(VkInstance vulkanInstance) {
    if (glfwCreateWindowSurface(vulkanInstance, glfwWindow, nullptr,
  ...
  void createSwapChain(VkPhysicalDevice physicalDevice, VkDevice logicalDevice,
    ...
    ltracer::QueueFamilyIndices indices =
        findQueueFamilies(physicalDevice, vulkanSurface);
  ...
  void recreateSwapChain(VkPhysicalDevice physicalDevice,
    ...
    createSwapChain(physicalDevice, logicalDevice, swapExtent,
                    swapChainSupport);
  }
