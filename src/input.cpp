                         ...
                         std::function<void()> callback)
{
	...
	CustomUserData& userData
	    = *reinterpret_cast<CustomUserData*>(glfwGetWindowUserPointer(glfwWindow));

	userData.keyListeners[key]
	    = {.triggerMode = triggerMode, .listeningMode = listeningMode, .callback = callback};
}
...
void handleInputCallback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	...
	for (const auto& [listenedKey, keyListener] : userData.keyListeners)
	{
		if (key == listenedKey)
		{
			bool uiActive = !userData.window.getMouseCursorCaptureEnabled();
			bool listeningModeMatches
			    = (keyListener.listeningMode == KeyListeningMode::UI_AND_FLYING_CAMERA)
			      || (keyListener.listeningMode == KeyListeningMode::UI_ONLY && uiActive)
			      || (keyListener.listeningMode == KeyListeningMode::FLYING_CAMERA && !uiActive);

			bool triggerModeMatches
			    = (keyListener.triggerMode == KeyTriggerMode::Repeat
			       && (action == GLFW_REPEAT || action == GLFW_PRESS))
			      || (keyListener.triggerMode == KeyTriggerMode::KeyDown && action == GLFW_PRESS)
			      || (keyListener.triggerMode == KeyTriggerMode::KeyUp && action == GLFW_RELEASE);

			if (listeningModeMatches && triggerModeMatches)
			{
